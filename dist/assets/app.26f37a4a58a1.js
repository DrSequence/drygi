...
function eventButton(e, full = false) {
  ...
  const b = node("button", undefined, `event ${e.type || "other"}${past ? " is-past" : ""}`);
  ...
  b.title = `${e.name} · ${locality} · ${countries[e.country] || e.country}${past ? " · Завершено" : ""}`;
  ...
  for (let offset = 0; offset < cells.length; offset += 7) {
    ...
    for (const date of dates) {
      ...
      const day = node("section", undefined,
        `day${date.getMonth() !== m ? " outside" : ""}${date.getDay() === 0 || date.getDay() === 6 ? " weekend" : ""}${key === today ? " today" : key < today ? " is-past" : ""}`);
      day.setAttribute("aria-label", fmt(key, { day: "numeric", month: "long", year: "numeric" }));
      const number = node("button", date.getDate(), "day-number");
      number.setAttribute("aria-label", `${fmt(key)}: все события${key < today ? ", прошедший день" : ""}`);
