...
function renderMonths(visibleEvents) {
  ...
  for (const month of options) {
    ...
    button.append(
      node("span", month.toLocaleDateString("ru-RU", { month: "long" })),
      node(
        "small",
        `${month.getFullYear()} · ${count} ${plural(count, "дрыг", "дрыга", "дрыг")}`,
      ),
    );
...
function render() {
  ...
  $("#month-count").textContent = count
    ? `${count} ${plural(count, "событие", "события", "событий")} ${selectedTypes.size || selectedCountries.size ? "по выбранным фильтрам" : "в этом месяце"} · Нажми на дрыг, чтобы узнать подробности`
...
function fitWeeks() {
  for (const week of $("#calendar").children) {
    ...
    week.style.gridTemplateRows = `22px ${capacity ? `repeat(${capacity}, ${compact ? "32px" : "72px"}) ` : ""}minmax(16px, 1fr)`;
